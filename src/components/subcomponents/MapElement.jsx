...
function MapElement() {
	...
	return isLoaded ? (
		<GoogleMap
			...
			{selected ? (
				<InfoWindow position={{ lat: selected.location.lat, lng: selected.location.lng }} options={{ pixelOffset: new window.google.maps.Size(0, -34) }} onCloseClick={onCloseClick}>
					<div className="d-flex flex-column align-items-center">
						<h6>{selected.name}</h6>
						<div>{selected.address1}</div>
						<div>{selected.address2}</div>
						<Link to={"/destination-" + selected.id} state={{ loc: selected }} className="btn btn-primary" style={{ fontSize: "8pt" }}>
							More Details {">>"}
						</Link>
					</div>
				</InfoWindow>
